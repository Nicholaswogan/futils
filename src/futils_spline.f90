module futils_spline
  ...
  ! This module is based on a code written by John E. Pask, LLNL.
    ...
    use futils_lapack, only: dgesv, dgbsv
    implicit none
    private
    public :: spline3
  ...
  contains
    ...
    function spline3(x, y, xnew, err) result(ynew)
  ...
  ! get spline parameters: 2nd derivs at ends determined by cubic interpolation
      call spline3pars(x, y, [2, 2], [0._dp, 0._dp], c, err)
      if (allocated(err)) return
  
      ip = 0
      do i = 1, size(xnew)
        ip = iixmin(xnew(i), x, ip, err)
        if (allocated(err)) return
        ynew(i) = poly3(xnew(i), c(:, ip))
      end do
    end function spline3
  
    subroutine spline3pars(xi, yi, bctype, bcval, c, err)
  ...
  ! check input parameters
      if (bctype(1) < 1 .or. bctype(1) > 2) then
        err = "spline3pars error: bctype /= 1 or 2."
        return
      end if
      if (bctype(2) < 1 .or. bctype(2) > 2) then
        err = "spline3pars error: bctype /= 1 or 2."
        return
      end if
      if (size(c, 1) /= 5) then
        err = "spline3pars error: size(c,1) /= 5."
        return
      end if
      if (size(c, 2) /= size(xi) - 1) then
        err = "spline3pars error: size(c,2) /= size(xi)-1."
        return
      end if
      if (size(xi) /= size(yi)) then
        err = "spline3pars error: size(xi) /= size(yi)"
        return
      end if
  ...
  ! left end
      if (bctype(1) == 2) then
        ...
        if (info /= 0) then
          err = "spline3pars error: dgesv error."
          return
        end if
        ce = bemat(:, 1)
        d2p1 = 2*ce(3)
  ...
  ! solve spline equations -- LAPACK band form
      ...
      if (info /= 0) then
        err = "spline3pars error: dgbsv error."
        return
      end if
      cs = bmat(:, 1)
  !write(*,*) cs(1:6)
  ...
  ! returns value of polynomial \sum_{i=1}^4 c(i) (x-c(0))^(i-1)
    ...
    integer function iixmin(x, xi, i_min, err) result(ip)
  ! Just like iix, but assumes that x >= xi(i_min)
    ...
    integer function iix(x, xi, err) result(i1)
      ...
      if (n < 2) then
        ...
        if (n == 2) then
          ...
          if (n == 3) then
            ...
            if (x <= xi(1)) then ! left end
              i1 = 1
            else
              if (x <= xi(2)) then ! first element
                i1 = 1
              else
                if (x <= xi(3)) then ! second element
                  i1 = 2
                else
                  if (x >= xi(n)) then ! right end
                    i1 = n - 1
                  else
